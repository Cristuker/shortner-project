...
export class UrlController {
    ...
    @Post("/shorten")
    @UseGuards(AuthGuard)
    @ApiHeader({ name: 'authorization', example: 'Bearer tokewxyz', description: 'Authorization token'})
    ...
    @ApiHeader({ name: 'authorization', example: 'Bearer tokewxyz', required: true, description: 'Authorization token' })
    ...
    @ApiResponse({ status: 404, description: 'Url not found, please check information and try again' })
    @ApiResponse({ status: 401, description: 'Unathorized'})
